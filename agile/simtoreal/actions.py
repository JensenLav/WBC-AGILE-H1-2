...
class ActionProcessor:
    ...
    def _parse_joint_values(
        self, values: list, yaml_joint_names: list[str], motor_idx_to_joint_name: list[str]
    ) -> torch.Tensor:
        """
        ...
        result = torch.zeros(len(motor_idx_to_joint_name), device=self.device, dtype=torch.float32)
        # Map values.
        for yaml_idx, joint_name in enumerate(yaml_joint_names):
            if joint_name in motor_idx_to_joint_name:
                real_idx = motor_idx_to_joint_name.index(joint_name)
                result[real_idx] = values[yaml_idx]
